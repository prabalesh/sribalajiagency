...
export class FileStorageService implements StorageProvider {
    ...
    async saveFile(file: Express.Multer.File, folder: string = 'images', optimize: boolean = true): Promise<string> {
        ...
        try {
            ...
            // Log operation start
            this.logger.log(`[R2-${operationId}] Starting file upload - Original: ${file.originalname}, Size: ${file.size} bytes, Optimize: ${optimize}`);
            ...
            if (optimize) {
                ...
                extension = 'webp';
                this.logger.log(`[R2-${operationId}] Sharp processing completed in ${Date.now() - sharpStart}ms, New size: ${buffer.length} bytes`);
            } else {
                ...
                this.logger.log(`[R2-${operationId}] Skipping optimization, using original buffer`);
            }
            ...
            this.logger.log(`[R2-${operationId}] CLASS A OPERATION: PutObject - Bucket: ${this.bucketName}, Key: ${key}, ContentType: ${contentType}, Size: ${buffer.length} bytes`);
            ...
            this.logger.log(`[R2-${operationId}] ✓ Upload SUCCESS - Key: ${key}, Upload time: ${uploadDuration}ms, Total time: ${totalDuration}ms, Total PutObject ops: ${r2OperationCounters.putObjectSuccess}`);
            ...
            const totalDuration = Date.now() - startTime;
            this.logger.error(`[R2-${operationId}] ✗ Upload FAILED - File: ${file.originalname}, Duration: ${totalDuration}ms, Error: ${error.message}, Total failures: ${r2OperationCounters.putObjectFailed}`, error.stack);
            ...
            if (error.$metadata?.attempts) {
                ...
                this.logger.warn(`[R2-${operationId}] Operation retried ${retryCount} times. Total retries: ${r2OperationCounters.totalRetries}`);
            }
    ...
    async deleteFile(fileUrl: string): Promise<void> {
        ...
        try {
            ...
            this.logger.log(`[R2-${operationId}] Starting file deletion - URL: ${fileUrl}`);

            if (this.publicUrl && fileUrl.startsWith(this.publicUrl)) {
                key = fileUrl.replace(`${this.publicUrl}/`, '');
                this.logger.log(`[R2-${operationId}] Extracted key from public URL: ${key}`);
                ...
                this.logger.log(`[R2-${operationId}] Extracted key from HTTP URL: ${key}`);
            }

            // Log DeleteObject operation details
            this.logger.log(`[R2-${operationId}] CLASS A OPERATION: DeleteObject - Bucket: ${this.bucketName}, Key: ${key}`);
            ...
            const totalDuration = Date.now() - startTime;
            this.logger.log(`[R2-${operationId}] ✓ Delete SUCCESS - Key: ${key}, Delete time: ${deleteDuration}ms, Total time: ${totalDuration}ms, Total DeleteObject ops: ${r2OperationCounters.deleteObjectSuccess}`);
        } catch (error) {
            // Increment failure counter
            r2OperationCounters.deleteObjectFailed++;

            const totalDuration = Date.now() - startTime;
            this.logger.error(`[R2-${operationId}] ✗ Delete FAILED - URL: ${fileUrl}, Duration: ${totalDuration}ms, Error: ${error.message}, Total failures: ${r2OperationCounters.deleteObjectFailed}`, error.stack);
