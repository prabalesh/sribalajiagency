...
import { UpdateCartDto, ValidateCartDto, MergeCartDto, CartItemDto } from './dto/cart.dto';
...
export class CartsController {
    ...
    async updateCart(@Request() req, @Body() updateCartDto: UpdateCartDto) {
        const cart = await this.cartsService.updateUserCart(req.user.id, updateCartDto.items);
        return this.cartsService.cartToDto(cart);
    }
    ...
    async mergeCart(@Request() req, @Body() mergeCartDto: MergeCartDto) {
        const cart = await this.cartsService.mergeGuestCart(req.user.id, mergeCartDto.guestCart);
        return this.cartsService.cartToDto(cart);
    }
